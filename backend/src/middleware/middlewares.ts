...
export const middlewareCheckProduto = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    const nome: string = req.body.nome
    const preco: number = req.body.preco
    const quantidade: number = req.body.quantidade
    const categoria: number = req.body.categoria
    ...
    if (!nome || !preco || !quantidade || !categoria) {
        return res.status(400).json({ message: "Atualize os dados da forma certa!" })
